...
static func build(seed: int, content: ContentDb) -> SimRoot:
	...
	if land.attach(sim) != OK:
		return null
	var standing: StandingSystem = StandingSystem.new(content, actors, events)
	...
	if portals.attach(sim, events) != OK:
		return null
	var movement: MovementSystem = MovementSystem.new(content, actors, land, build, items, stats, standing)
...
static func hacks_of(sim: SimRoot) -> HackSystem:
	var system: SimSystem = sim.get_system(HackSystem.SYSTEM_ID)
	if system == null:
		push_error("SimAssembly: sim has no '%s' system" % HackSystem.SYSTEM_ID)
		return null
