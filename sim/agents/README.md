...
| `corpse_system.gd` | `CorpseSystem` (system id `corpses`): one corpse entity per `actor.died`, holding the dead actor's whole inventory in `corpse.<id>`; `corpse.loot` moves it to a living actor within arm's reach, all of it or none. Death and looting are both a single whole-container move, so no item is created or destroyed by either (M6 spec claim 10; ADR-007 C). |
