...
func test_real_content_loads_and_registers_every_stat() -> void:
	...
	assert_eq(sim.system_ids(), [&"content", &"entities", &"stats", &"items", &"actors", &"land", &"combat", &"progression"] as Array[StringName], "fixed system order")
...
func test_commands_are_registered_by_the_item_system() -> void:
	...
	assert_eq(sim.commands().kinds(), [&"actor.spawn", &"actor.wield", &"item.spawn", &"land.grant", &"magazine.load", &"magazine.unload",
