...
func test_real_content_loads_and_registers_every_stat() -> void:
	...
	assert_eq(sim.system_ids(), [&"content", &"entities", &"stats", &"items", &"actors", &"combat", &"progression", &"land", &"structures", &"build", &"portals", &"movement", &"raids", &"perception", &"aim"] as Array[StringName], "fixed system order")
