...
func test_real_content_loads_and_registers_every_stat() -> void:
	...
	assert_eq(sim.system_ids(), [&"content", &"entities", &"stats", &"items", &"actors", &"combat", &"progression", &"land", &"standing", &"structures", &"build", &"portals", &"movement", &"raids", &"breaches", &"perception", &"aim", &"stress", &"pathing", &"squads", &"stances", &"quests", &"sensors", &"hacks", &"sites"] as Array[StringName], "fixed system order")
...
func test_commands_are_registered_by_the_item_system() -> void:
	...
	assert_eq(sim.commands().kinds(), [&"actor.climb", &"actor.equip_device", &"actor.move", &"actor.spawn", &"actor.wield", &"agent.set_profile", &"agent.spawn", &"build.breach", &"build.place", &"build.remove", &"hack.logout", &"hack.spoof", &"hack.start", &"item.attach", &"item.detach", &"item.spawn", &"land.identify", &"land.transfer", &"magazine.load", &"magazine.unload", &"module.install", &"module.remove", &"perk.unlock", &"quest.abandon", &"quest.accept", &"raid.spawn", &"sim.pause", &"sim.resume", &"site.raise", &"structure.place", &"weapon.attach", &"weapon.detach", &"weapon.fire", &"weapon.reload_emergency", &"weapon.reload_tactical"] as Array[StringName], "lexical kinds")
