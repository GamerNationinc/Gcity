...
func test_the_street_is_watched_and_the_rooms_are_not_seen_from_it() -> void:
	...
	var room_a: Vector3i = BuildSystem.cell_centre(M4Building.BASE + Vector3i(-1, 0, 7))
	var room_b: Vector3i = BuildSystem.cell_centre(M4Building.BASE + Vector3i(4, 0, 7))
	var door_a: Vector3i = BuildSystem.cell_centre(M4Building.BASE + Vector3i(1, 0, 8))
	var door_b: Vector3i = BuildSystem.cell_centre(M4Building.BASE + Vector3i(3, 0, 8))
	...
	var outside_a: Vector3i = BuildSystem.cell_centre(M4Building.BASE + Vector3i(-4, 0, 8))
	var behind_window_a: Vector3i = BuildSystem.cell_centre(M4Building.BASE + Vector3i(0, 0, 8))
	assert_true(perception.line_of_sight(outside_a, behind_window_a), "room A's window is seen through from outside")
	assert_false(perception.line_of_sight(outside_a, room_a), "the wall beside the window is not")
